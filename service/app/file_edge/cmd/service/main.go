...
var (
	...
	Name string = "thjam.file_edge.service"
...
func init() {
	...
	flag.StringVar(&temporaryFilePath, "tmp", "../../tmp", "temporary file path, eg: -tmp /tmp/file_edge")
}
...
func main() {
	...
	// 初始化临时目录
	env := conf.GetEnv()
	env.TemporaryFileDirectory = temporaryFilePath

	// 目录是否存在&创建
	_, err := os.Stat(env.TemporaryFileDirectory)
	if err != nil {
		if os.IsNotExist(err) {
			err := os.Mkdir(env.TemporaryFileDirectory, os.ModePerm)
			if err != nil {
				panic(err)
			}
		}
	...
	// 删除临时文件储存目录下的所有文件
	files, err := ioutil.ReadDir(env.TemporaryFileDirectory)
	if err != nil {
		panic(err)
	}
	for _, f := range files {
		if f.IsDir() {
			err := os.RemoveAll(env.TemporaryFileDirectory + "/" + f.Name())
			if err != nil {
				panic(err)
			}
		} else {
			err := os.Remove(env.TemporaryFileDirectory + "/" + f.Name())
			if err != nil {
				panic(err)
			}
		}
	...
	app, cleanup, err := initApp(pkgBootstrap.Server, pkgBootstrap.Service, bc.EdgeFile, rr, rd, logger)
	if err != nil {
		panic(err)
	}
	defer cleanup()
